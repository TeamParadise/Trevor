...
public class VisionTargetSpotter extends Subsystem
{
	...
	// The following string constants are for the smart dashboard:
	...
	final String particleCount = "Particle Count";
	final String targetTotalScore = "Target Total Score";
    final String targetLeftScore = "Target Left Score";
    final String targetRightScore = "Target Right Score";
    final String targetTapeWidthScore = "Target Tape Width Score";
    final String targetVerticalScore = "Target Vertical Score";
	final String targetDistance = "Target Distance";
        
    final String anyTargetHot = "Any Target Hot";
    final String shootingTargetHot = "Shooting Target Hot";

    final String shootingLeft = "Shooting Left";
    
    final String useRGB = "Use RGB";
	
	// End of smart dashboard constants
	...
	public void analyzeCameraImage()
	{
		if (null == camera)
		{
			// Could not instantiate the camera
			return;
		}
		...
		try
		{
			...
			SmartDashboard.putNumber(particleCount, filteredImage.getNumberParticles());
			...
			if (filteredImage.getNumberParticles() > 0)
			{
				...
				for (int i = 0; i < verticalTargetCount; i++)
				{
					...
					for (int j = 0; j < horizontalTargetCount; j++)
					{
						...
						//Determine if the width of the tape o`n the two targets appears to be the same
	...
	public boolean isDesiredTargetHot()
	{
    ...
    private void updateSmartDashboard(TargetReport target)
    {
        ...
        SmartDashboard.putNumber(targetRightScore, target.rightScore);
        SmartDashboard.putNumber(targetTapeWidthScore, target.tapeWidthScore);
        SmartDashboard.putNumber(targetVerticalScore, target.verticalScore);
        
        SmartDashboard.putBoolean(anyTargetHot, target.Hot);
        SmartDashboard.putBoolean(shootingTargetHot, target.Hot &&
    ...
    double scoreRectangularity(ParticleAnalysisReport report)
    {
        return (report.boundingRectWidth*report.boundingRectHeight != 0)
            ? 100*report.particleArea/(report.boundingRectWidth*report.boundingRectHeight)
            : 0;
    }
